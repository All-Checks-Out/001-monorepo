...
export async function selectLocalUser(page: Page, email: string, path = "/") {
  await test.step(`Select local user ${displayUser(email)}`, async () => {
...
export async function clearLocalUser(page: Page) {
  await test.step("Clear any selected local user", async () => {
    await page.goto("/");
...
function displayUser(email: string) {
  if (email === associationUserEmail) return "Prudence";
  if (email === providerUserEmail) return "Arthur";
  return email;
}
