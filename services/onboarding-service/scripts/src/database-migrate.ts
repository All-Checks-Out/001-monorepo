...
async function connectWithRetry(credentials: DbCredentials, database: string) {
  ...
  for (let attempt = 1; attempt <= CONNECT_MAX_ATTEMPTS; attempt++) {
    const client = new Client({
      ...
      ssl: credentials.ssl === false ? undefined : { rejectUnauthorized: false },
...
async function main() {
  ...
  const sslMode = credentials.ssl === false ? "disable" : "require";
  const jdbcUrl = `jdbc:postgresql://${credentials.host}:${credentials.port ?? 5432}/${databaseName}?sslmode=${sslMode}`;

  const flywayArgs = [
    `-connectRetries=${FLYWAY_RETRIES}`,
    `-url=${jdbcUrl}`,
    `-user=${credentials.username}`,
    `-password=${credentials.password}`,
    `-locations=filesystem:${sqlDir}`,
    "-schemas=onboarding,document_analysis,public",
    ...(cleanDisabledFlag ? [cleanDisabledFlag] : []),
    flywayCommand,
  ];

  execFileSync("flyway", flywayArgs, { stdio: "inherit", cwd: rootDir });
